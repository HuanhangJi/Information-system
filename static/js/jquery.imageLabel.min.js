	...
	function a() {
		...
		$(".imageLabel-content").mousedown(function(e) {
			if (d = e.button, 2 != e.button) {
				...
				$(e.target).hasClass("imageLabel-imgdrop")) p = !0,
				s = $(e.target),
		...
		$(".imageLabel-img-boxs").mousemove(function(e) {
			...
			if (b) {
				...
				Math.abs(a.clientX - t.cx) > 10 && Math.abs(a.clientY - t.cy) > 10 && !p && !v ? (setTimeout(function() {
		...
		var y = $(".imageLabel-input"),u = y.find("input");
		...
		y.find(".imageLabel-input-ok").click(function() {
			...
			e.name = u.val(),
			s.attr("data-json", JSON.stringify(e)),
			y.removeClass("imageLabel-active"),
			u.val() ? s.addClass("imageLabel-drop-has") : s.removeClass("imageLabel-drop-has")
		}),
		...
		$(".imageLabel-closes").click(function() {
			...
			500)) && $('.imageLabel-box').remove()
		}).next().click(function() {
			l.removeClass("imageLabel-box-active")
			n.confirm(i.getData()) && l.removeClass("imageLabel-box-active")
			$('.imageLabel-box').remove();
		})
	}
	if (!e.img) return alert("请填写图片地址"),
	!1;
	var i = {
		getData: function() {
			var e = [];
